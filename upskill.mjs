#!/usr/bin/env node
/**
 * upskill.mjs — Aggregate skill-gap analyzer for job-hunter-ai (#1520, phase 1)
 *
...
const CV_FILE = join(job_hunter_ai, 'cv.md');
const PROFILE_FILE = join(job_hunter_ai, 'config/profile.yml');
...
function analyze(minReports) {
  ...
  for (const row of rows) {
    ...
    const candidates = [join(dirname(APPS_FILE), linkMatch[1]), join(job_hunter_ai, linkMatch[1])];
...
if (urlTextIdx !== -1 || directUrl) {
  (async () => {
    ...
    if (!existsSync(activeCvFile)) {
      activeCvFile = join(job_hunter_ai, 'cv-example.md');
    }
    if (existsSync(activeCvFile)) {
