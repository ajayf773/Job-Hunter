...
> Claude Code marketplace metadata). These plugins extend job-hunter-ai itself.
...
A plugin is a directory under `plugins/` (bundled, shipped with job-hunter-ai) or
...
job-hunter-ai is plain ESM with no build step, so the engine **cannot truly
...
- **Centralized infrastructure** the project would operate — hosted job
  ...
  [Where job-hunter-ai is going (#904)](https://github.com/santifer/job-hunter-ai/discussions/904) —
  not the open-core.
- **Auto-submitting / blind-applying** to jobs. job-hunter-ai is a decision-support
