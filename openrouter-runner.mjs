#!/usr/bin/env node
/**
 * job-hunter-ai OpenRouter Runner
...
async function callOpenRouter(systemPrompt, userMessage) {
  ...
  const pinnedModel = process.env.job_hunter_ai_MODEL;
  if (pinnedModel) {
    ...
    try {
      const resp = await fetch(OPENROUTER_API_URL, {
        ...
        headers: {
          ...
          'HTTP-Referer':  'https://github.com/santifer/job-hunter-ai',
...
async function fetchJobPage(url) {
  ...
  try {
    const r = await fetch(url, {
      headers: { 'User-Agent': 'Mozilla/5.0 (compatible; job-hunter-ai/1.0)' }
    });
...
// search-query companies are handled by the full /job-hunter-ai scan pipeline.
...
if (invokedDirectly && ['evaluate', 'eval', 'pipeline', 'apply', 'models'].includes(command) && !process.env.job_hunter_ai_MODEL) {
...
MODEL SELECTION:
  ...
  - Pin a model:  job_hunter_ai_MODEL=deepseek/deepseek-r1:free node openrouter-runner.mjs eval <url>
`);
}

if (invokedDirectly && ['scan', 'evaluate', 'eval', 'pipeline', 'apply'].includes(command)) {
  const modelName = process.env.job_hunter_ai_MODEL || activeModel || 'free-rotation';
