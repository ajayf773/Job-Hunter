...
export function loadBenchmarks(explicitPath) {
  const path = explicitPath
    || (existsSync(join(job_hunter_ai, 'config/benchmarks.yml')) ? join(job_hunter_ai, 'config/benchmarks.yml') : join(job_hunter_ai, 'templates/benchmarks.yml'));
  let doc;
  try {
...
function selfTest() {
  ...
  const bm = loadBenchmarks(join(job_hunter_ai, 'templates/benchmarks.yml')).benchmarks;
...
function main() {
  ...
  const states = loadCanonicalStates(STATES_FILE);
  const trackerPath = resolveTrackerPath(job_hunter_ai);
