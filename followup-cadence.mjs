#!/usr/bin/env node
/**
 * followup-cadence.mjs — Follow-up Cadence Tracker for job-hunter-ai
 *
...
const APPS_FILE = existsSync(join(job_hunter_ai, 'data/applications.md'))
  ? join(job_hunter_ai, 'data/applications.md')
  : join(job_hunter_ai, 'applications.md');
const FOLLOWUPS_FILE = join(job_hunter_ai, 'data/follow-ups.md');
const PROFILE_FILE = process.env.job_hunter_ai_PROFILE || join(job_hunter_ai, 'config/profile.yml');
...
// --- Resolve report path ---
export function resolveReportPath(reportField, appsFile = APPS_FILE, repoRoot = job_hunter_ai) {
