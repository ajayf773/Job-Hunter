 ...
 *
 * Usage:
 *   node job-hunter-ai/generate-pdf.mjs <input.html> <output.pdf> [--format=letter|a4] [--report=NNN] [--allow-reorder] [--max-pages=N] [--strict-pages]
 *
 ...
 * or outside the job-hunter-ai repository.
 *
 ...
 * job-hunter-ai root with forward slashes. One row per PDF path; when a report
...
async function generatePDF() {
  ...
  if (!inputPath || !outputPath) {
    ...
    console.error('mechanical markdown-to-HTML step by design. Run `/job-hunter-ai pdf` in your AI');
...
export async function renderHtmlToPdf(html, outputPath, opts = {}) {
  ...
  const tmpHtmlPath = resolve(baseDir, `.job-hunter-ai-render-${randomUUID()}.html`);
