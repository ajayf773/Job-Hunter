 ...
 * next `/job-hunter-ai pipeline` run both re-surface them, and they get evaluated
...
function resolveInsideRepo(inputPath, fallbackPath, flag) {
  ...
  try {
    repoReal = realpathSync(job_hunter_ai);
...
const defaultPipeline = existsSync(join(job_hunter_ai, 'data/pipeline.md'))
  ? join(job_hunter_ai, 'data/pipeline.md')
  : join(job_hunter_ai, 'pipeline.md');
const PIPELINE_FILE = resolveInsideRepo(argValue('--pipeline'), defaultPipeline, '--pipeline');
const STATE_FILE = resolveInsideRepo(argValue('--state'), join(job_hunter_ai, 'batch/batch-state.tsv'), '--state');
const REPORTS_DIR = join(job_hunter_ai, 'reports');
...
for (let i = pendStart + 1; i < pendEnd; i++) {
  ...
  const reportLink = normalizeReportLink(`[${num}](reports/${reportFile})`, dirname(PIPELINE_FILE), job_hunter_ai);
