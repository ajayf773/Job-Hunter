...
const dir = mkdtempSync(join(tmpdir(), 'job-hunter-ai-guard-'));
...
try {
  ...
  //    range instead of waiting out the lock's real default timeout.
  {
    ...
    writeFileSync(p, HEADER + '2026-01-01\t' + marker + '\treachable\n', 'utf-8');
    const prevTimeout = process.env.job_hunter_ai_PORTAL_HEALTH_LOCK_TIMEOUT_MS;
    const prevRetry = process.env.job_hunter_ai_PORTAL_HEALTH_LOCK_RETRY_MS;
    process.env.job_hunter_ai_PORTAL_HEALTH_LOCK_TIMEOUT_MS = '200';
    process.env.job_hunter_ai_PORTAL_HEALTH_LOCK_RETRY_MS = '20';
    ...
    try {
      ...
      held.release();
      if (prevTimeout === undefined) delete process.env.job_hunter_ai_PORTAL_HEALTH_LOCK_TIMEOUT_MS;
      else process.env.job_hunter_ai_PORTAL_HEALTH_LOCK_TIMEOUT_MS = prevTimeout;
      if (prevRetry === undefined) delete process.env.job_hunter_ai_PORTAL_HEALTH_LOCK_RETRY_MS;
      else process.env.job_hunter_ai_PORTAL_HEALTH_LOCK_RETRY_MS = prevRetry;
    }
  }
} finally {
