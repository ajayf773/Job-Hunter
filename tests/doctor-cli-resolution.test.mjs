...
try {
  // 1. --cli flag beats process.env.job_hunter_ai_CLI. Pin precedence at the
  //    flag/env boundary so a future reorder of resolveActiveCli fails loudly.
  {
    const dir = mkdtempSync(join(tmpdir(), 'co-resolve-1-'));
    try {
      const state = runDoctor(dir, ['--cli', 'opencode'], { job_hunter_ai_CLI: 'claude' });
      if (expectOk(state, '#1 flag beats env')
          && state.active_cli === 'opencode'
          && state.cli_source === 'flag') {
        pass('--cli flag beats job_hunter_ai_CLI env (active_cli=opencode, cli_source=flag)');
      } else {
  ...
  // 2. process.env.job_hunter_ai_CLI beats job_hunter_ai_CLI in .env. Pin
  //    precedence at the env/.env boundary.
  {
    const dir = mkdtempSync(join(tmpdir(), 'co-resolve-2-'));
    try {
      writeFileSync(join(dir, '.env'), 'job_hunter_ai_CLI=claude\n');
      const state = runDoctor(dir, [], { job_hunter_ai_CLI: 'opencode' });
      if (expectOk(state, '#2 env beats .env')
          && state.active_cli === 'opencode'
          && state.cli_source === 'env') {
        pass('process.env job_hunter_ai_CLI beats .env file (active_cli=opencode, cli_source=env)');
      } else {
