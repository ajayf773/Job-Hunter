...
try {
  ...
  const dir = mkdtempSync(join(tmpdir(), 'job-hunter-ai-theme-'));
  try {
  ...
  if (block.includes('id="job-hunter-ai-dynamic-theme"') && block.includes(':root {') && block.includes('--accent-color: #2563eb;') && block.includes('--font-size: 10pt;')) {
  ...
  if (injected.includes('job-hunter-ai-dynamic-theme') && injected.indexOf('job-hunter-ai-dynamic-theme') < injected.indexOf('</head>') && injected.indexOf('job-hunter-ai-dynamic-theme') > injected.indexOf('<style>')) {
  ...
  if (noHead.startsWith('<style id="job-hunter-ai-dynamic-theme"')) pass('injectThemeStyle prepends the block when there is no </head>');
  ...
  // a --page-margin override earlier in <head> is what actually wins.
  {
    ...
    const overrideIdx = withOverride.indexOf('job-hunter-ai-dynamic-theme'); // the profile's style.margin override
    const pageSetupIdx = withOverride.indexOf('job-hunter-ai-page-setup');   // injectPrintPageCss's @page rule
