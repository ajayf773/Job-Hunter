#!/usr/bin/env node
/**
 * add-entry.mjs — Deterministic dedup + insertion for `/job-hunter-ai add`.
 *
 ...
 *
 * Test isolation: job_hunter_ai_CV / job_hunter_ai_ARTICLE_DIGEST override the target
...
const CV_FILE = process.env.job_hunter_ai_CV || join(job_hunter_ai, 'cv.md');
const ARTICLE_DIGEST_FILE = process.env.job_hunter_ai_ARTICLE_DIGEST || join(job_hunter_ai, 'article-digest.md');
...
export function applyAdd(payload, { cvText = null, articleText = null } = {}) {
  ...
  if (payload.articleDigest) {
    ...
    const current = articleText === null
      ? '# Article Digest -- Proof Points\n\nCompact proof points from portfolio projects. Read by job-hunter-ai at evaluation time.\n'
      : articleText;
