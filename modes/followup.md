...
(`/job-hunter-ai email noshow`) instead.
...
> "No active applications to follow up on. Apply to some roles first with `/job-hunter-ai` and come back when they're aging."
...
- Suggest the user run `/job-hunter-ai contacto {company}` to find the right person first
...
> - Trying a different contact via `/job-hunter-ai contacto`
...
**To:** {email or "No contact found — run `/job-hunter-ai contacto` first"}
