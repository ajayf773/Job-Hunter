...
the public JSON APIs job-hunter-ai already supports — Greenhouse, Ashby, Lever, and
...
- `portals.yml` — dedupe target and write destination (user layer). Honors the
  `job_hunter_ai_PORTALS` env override for scratch/testing.
...
- **Genuinely unknown**: for a JS-only portal with no ATS API, paste a specific
  JD into `data/pipeline.md` and run `/job-hunter-ai pipeline`.

## Step 3 — Handoff

After writing, tell the user to run `/job-hunter-ai scan` (or a regional preset
