...
1. `/job-hunter-ai email {report-number-or-slug}`
   ...
   - If `data/pdf-index.tsv` contains a PDF for that report, mention it as the CV
     ...
     generated first via `/job-hunter-ai pdf {slug}` or attached manually.

2. `/job-hunter-ai email {pasted JD}`
...
4. `/job-hunter-ai email stuck {report-number-or-slug}`
...
5. `/job-hunter-ai email noshow {report-number-or-slug}`
   - Load the matching `reports/{NNN}-*.md` for company and role context.
...
- `< 4.0`: restrained; do not oversell. If below 4.0, warn the user before
  drafting that job-hunter-ai normally recommends against applying.
