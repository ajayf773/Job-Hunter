...
Works in two modes:
- **Slug mode:** `/job-hunter-ai cover {slug}` — loads the existing evaluation report draft as a starting point
- **Paste mode:** `/job-hunter-ai cover` or JD pasted directly — starts from scratch
...
When invoked as `/job-hunter-ai cover {slug}`:
