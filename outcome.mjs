...
const SET_STATUS_SCRIPT = join(job_hunter_ai, 'set-status.mjs');
const ARCHIVE_POSTING_SCRIPT = join(job_hunter_ai, 'archive-posting.mjs');
...
const appsFile = resolveTrackerPath(job_hunter_ai);
...
try {
  const statusOutput = execFileSync(NODE, setStatusArgs, { cwd: job_hunter_ai, env: process.env, encoding: 'utf-8' });
