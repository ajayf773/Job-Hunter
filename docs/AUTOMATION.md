...
`job-hunter-ai` offers to scan for you on a schedule ("just say *scan every 3 days*"),
...
> local files. Evaluating a shortlisted role later (`/job-hunter-ai pipeline`) is the
...
Replace `/path/to/job-hunter-ai` with your checkout path, and make sure `node` is on
...
```cron
0 9 */3 * * cd /path/to/job-hunter-ai && /usr/local/bin/node scan.mjs >> data/scan.log 2>&1
```
...
```cron
0 8 * * 1-5 cd /path/to/job-hunter-ai && /usr/local/bin/node scan.mjs >> data/scan.log 2>&1
```

### macOS — launchd (survives sleep better than cron)

Save as `~/Library/LaunchAgents/io.job-hunter-ai.scan.plist`, then
`launchctl load ~/Library/LaunchAgents/io.job-hunter-ai.scan.plist`:
...
<plist version="1.0">
<dict>
  <key>Label</key>            <string>io.job-hunter-ai.scan</string>
  <key>ProgramArguments</key>
  <array>
    <string>/usr/local/bin/node</string>
    <string>scan.mjs</string>
  </array>
  <key>WorkingDirectory</key> <string>/path/to/job-hunter-ai</string>
  <key>StartCalendarInterval</key>
  <dict>
    <key>Hour</key>    <integer>9</integer>
    <key>Minute</key>  <integer>0</integer>
  </dict>
  <key>StandardOutPath</key>   <string>/path/to/job-hunter-ai/data/scan.log</string>
  <key>StandardErrorPath</key> <string>/path/to/job-hunter-ai/data/scan.log</string>
</dict>
</plist>
```
...
```powershell
$action  = New-ScheduledTaskAction -Execute "node.exe" -Argument "scan.mjs" -WorkingDirectory "C:\path\to\job-hunter-ai"
$trigger = New-ScheduledTaskTrigger -Daily -At 9am
Register-ScheduledTask -TaskName "job-hunter-ai scan" -Action $action -Trigger $trigger -Description "Recurring job-hunter-ai job scan"
```
...
## How this fits the rest of job-hunter-ai
