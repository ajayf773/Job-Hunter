...
- **Listed** — one `plugins-registry/<id>.json` file; the code stays in the
  author's `job-hunter-ai-plugin-<name>` repo; users `add` it. The default, low
  burden.
...
- **Naming + identity:** repo is `job-hunter-ai-plugin-<name>`, `id` == name minus
...
- **Wording (public-forever):** description / README / skill carry no commercial
  / hosted-service / monetization language. job-hunter-ai is free and local-first;
...
**not** registry-listed. It can still be a `job-hunter-ai-plugin-<name>` repo users
...
feature PRs against it — close-redirect them to "publish `job-hunter-ai-plugin-<id>`
