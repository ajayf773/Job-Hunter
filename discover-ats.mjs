#!/usr/bin/env node
/**
 * discover-ats.mjs — Company-list → scannable ATS board resolver for job-hunter-ai
 *
 * Takes a list of companies and resolves each to a scannable ATS board by
 * probing the public JSON APIs job-hunter-ai already supports (Greenhouse, Ashby,
 ...
 *
 * Probing hits live third-party APIs, so honor job_hunter_ai_PORTALS to point at a
 * scratch portals file during tests/experiments.
 *
 * Issue #1864 — github.com/santifer/job-hunter-ai
 */
...
const PORTALS_PATH = process.env.job_hunter_ai_PORTALS || join(job_hunter_ai, 'portals.yml');
...
function printSummary({ resolved, unresolved, duplicates }) {
  console.log(`\n${'='.repeat(78)}`);
  console.log('  ATS Discovery — job-hunter-ai');
...
async function main() {
  ...
  // rest of job-hunter-ai treats user files (see DATA_CONTRACT.md).
  let written = false;
