 ...
 *
 * Provisions a throwaway queue via job_hunter_ai_INBOX and a temp CWD; never
...
function run(inbox, args, opts = {}) {
  return execFileSync(NODE, [CLI, ...args], {
    cwd: ROOT,
    env: { ...process.env, job_hunter_ai_INBOX: inbox },
    encoding: 'utf8',
...
console.log('6. first add on the default path self-heals .gitignore (idempotent)');
{
  ...
  const addOnce = () => execFileSync(NODE, [CLI, 'add', 'queue a scan'], {
    cwd: repo, env: { ...process.env, job_hunter_ai_INBOX: '' }, stdio: ['pipe', 'pipe', 'pipe'],
  });
  addOnce(); addOnce();
