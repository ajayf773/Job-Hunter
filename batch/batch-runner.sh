...
# job-hunter-ai batch runner — standalone orchestrator for claude -p workers
...
usage() {
  cat <<'USAGE'
job-hunter-ai batch runner — process job offers in batch via claude -p workers
...
main() {
  ...
  echo "=== job-hunter-ai batch runner ==="
  if (( LIMIT > 0 )); then
