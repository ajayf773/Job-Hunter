 ...
 * *acting on* it.
 *
 * job-hunter-ai is driven from an AI session, but there's no durable place to drop
...
const PATH = process.env.job_hunter_ai_INBOX || 'data/agent-inbox.md';

const HEADER = [
  '# Agent Inbox',
  '',
  '> **Agent protocol:** at the start of a job-hunter-ai session, read this file.',
...
function ensureGitignored() {
  ...
  if (process.env.job_hunter_ai_INBOX || PATH !== 'data/agent-inbox.md') return;
  try {
