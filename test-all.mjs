...
/**
 * test-all.mjs — Comprehensive test suite for job-hunter-ai
 *
...
if (ONLY !== null) {
  ...
  console.log('\n🧪 job-hunter-ai test suite (--only ' + ONLY + ')\n');
  await runDiscovered(ONLY);
  finish();
}

console.log('\n🧪 job-hunter-ai test suite\n');
...
const scripts = [
  ...
  // tests inside their active job-hunter-ai workspace.
...
try {
  const tmp = mkdtempSync(join(tmpdir(), 'job-hunter-ai-cv-facts-'));
...
if (!QUICK) {
  ...
  if (!hasGo) {
    ...
    if (isWindows && !goEnv.GOCACHE) {
      goEnv.GOCACHE = join(tmpdir(), 'job-hunter-ai-go-build-cache');
    }
    if (goEnv.GOCACHE) {
...
const systemFiles = [
  ...
  '.claude/skills/job-hunter-ai/SKILL.md',
  '.cursor/skills/job-hunter-ai/SKILL.md',
  '.opencode/skills/job-hunter-ai/SKILL.md',
  '.qwen/skills/job-hunter-ai/SKILL.md',
  '.antigravitycli/skills/job-hunter-ai/SKILL.md',
  '.grok/skills/job-hunter-ai/SKILL.md',
  '.kimi/skills/job-hunter-ai/SKILL.md',
];
...
const CANONICAL_ENTRYPOINT = '.agents/skills/job-hunter-ai/SKILL.md';
...
const skillEntrypoints = systemFiles.filter((f) => f.endsWith('/skills/job-hunter-ai/SKILL.md'));
...
try {
  ...
  if (
    ...
    injectedPageCss.indexOf('job-hunter-ai-page-setup') < injectedPageCss.indexOf('</head>')
  ) {
  ...
  if (fragmentPageCss.startsWith('<style id="job-hunter-ai-page-setup">')) {
...
try {
  ...
  const fixtureRoot = mkdtempSync(join(tmpdir(), 'job-hunter-ai-pdf-cleanup-launch-'));
  ...
  const leftovers = readdirSync(fixtureRoot)
    .filter((name) => name.startsWith('.job-hunter-ai-render-'));
...
try {
  const { renderHtmlToPdf } = await import(pathToFileURL(join(ROOT, 'generate-pdf.mjs')).href);
  const fixtureRoot = mkdtempSync(join(tmpdir(), 'job-hunter-ai-pdf-cleanup-page-'));
...
const careerOpsSkill = readTextLF('.agents/skills/job-hunter-ai/SKILL.md');
...
if (/language\.output/.test(careerOpsSkill) && /human-facing output/i.test(careerOpsSkill)) {
  pass('job-hunter-ai skill injects the output language rule');
} else {
  fail('job-hunter-ai skill does not inject the output language rule');
}
...
for (const skillPath of ['.claude/skills/job-hunter-ai/SKILL.md', '.agents/skills/job-hunter-ai/SKILL.md']) {
  if (!fileExists(skillPath)) {
    fail(`${skillPath} is missing`);
    continue;
  }
  const skill = readFile(skillPath);
  if (skill.includes('/job-hunter-ai latex')) {
    pass(`${skillPath} exposes /job-hunter-ai latex in discovery menu`);
  } else {
    fail(`${skillPath} does not expose /job-hunter-ai latex in discovery menu`);
  }
  if (
    skill.includes('email') &&
    skill.includes('| `email` | `email` |') &&
    skill.includes('/job-hunter-ai email') &&
    /Standalone modes[\s\S]*Applies to:[^\n]*`email`/.test(skill)
  ) {
    pass(`${skillPath} exposes /job-hunter-ai email in routing, discovery, and standalone loading`);
  } else {
    fail(`${skillPath} does not fully expose /job-hunter-ai email`);
  }
}
...
const routerSkill = readFile('.agents/skills/job-hunter-ai/SKILL.md');
if (
  /argument-hint:.*offer-prep/.test(routerSkill) &&
  routerSkill.includes('| `offer-prep` | `offer-prep` |') &&
  routerSkill.includes('/job-hunter-ai offer-prep') &&
...
// Rule 8 — scope boundary: link to /job-hunter-ai training; never run training's scoring.
if (
  upskillModeDoc.includes('/job-hunter-ai training {name}') &&
  upskillModeDoc.includes('6-dimension scoring') &&
  upskillModeDoc.includes('`upskill` finds; `training` judges')
) {
  pass('upskill trust rule 8: scope boundary — links to /job-hunter-ai training, never runs training scoring');
} else {
...
try {
  ...
  const fixtureRoot = mkdtempSync(join(tmpdir(), 'job-hunter-ai-missing-pipeline-'));
...
try {
  ...
  const fixtureRoot = mkdtempSync(join(tmpdir(), 'job-hunter-ai-pipeline-lock-'));
  const originalCwd = process.cwd();
  let prevTimeout;
  let prevRetry;
  try {
    ...
    prevTimeout = process.env.job_hunter_ai_PIPELINE_LOCK_TIMEOUT_MS;
    prevRetry = process.env.job_hunter_ai_PIPELINE_LOCK_RETRY_MS;
    process.env.job_hunter_ai_PIPELINE_LOCK_TIMEOUT_MS = '200';
    process.env.job_hunter_ai_PIPELINE_LOCK_RETRY_MS = '20';
    ...
    if (prevTimeout === undefined) delete process.env.job_hunter_ai_PIPELINE_LOCK_TIMEOUT_MS;
    else process.env.job_hunter_ai_PIPELINE_LOCK_TIMEOUT_MS = prevTimeout;
    if (prevRetry === undefined) delete process.env.job_hunter_ai_PIPELINE_LOCK_RETRY_MS;
    else process.env.job_hunter_ai_PIPELINE_LOCK_RETRY_MS = prevRetry;
    process.chdir(originalCwd);
    rmSync(fixtureRoot, { recursive: true, force: true });
  }
} catch (err) {
...
try {
  ...
  const fixtureRoot = mkdtempSync(join(tmpdir(), 'job-hunter-ai-seen-urls-'));
  const originalCwd = process.cwd();
  try {
    mkdirSync(join(fixtureRoot, 'data'), { recursive: true });
    writeFileSync(
...
try {
  ...
  const fixtureRoot = mkdtempSync(join(tmpdir(), 'job-hunter-ai-blacklist-'));
  try {
...
try {
  const tmp = mkdtempSync(join(tmpdir(), 'job-hunter-ai-portals-validator-'));
...
try {
  ...
  const dryRunTmp = mkdtempSync(join(tmpdir(), 'job-hunter-ai-fix-slugs-dryrun-'));
...
const claudeSkillEntrypoint = readFile('.claude/skills/job-hunter-ai/SKILL.md');
if (/\.agents\/skills\/job-hunter-ai\/SKILL\.md/.test(claudeSkillEntrypoint) || claudeSkillEntrypoint === readFile('.agents/skills/job-hunter-ai/SKILL.md')) {
  pass('Claude skill invocation resolves to the canonical job-hunter-ai router');
} else {
  fail('Claude skill invocation does not resolve to the canonical job-hunter-ai router');
}
...
const canonicalSkill = '.agents/skills/job-hunter-ai/SKILL.md';
const symlinks = [
  '.claude/skills/job-hunter-ai/SKILL.md',
  '.cursor/skills/job-hunter-ai/SKILL.md',
  '.opencode/skills/job-hunter-ai/SKILL.md',
...
if (
  ...
  /\/job-hunter-ai/.test(canonicalContent ?? '')
) {
  pass('job-hunter-ai skill router documents the Codex invocation model');
} else {
  fail('job-hunter-ai skill router is missing Codex invocation guidance');
}
...
if (
  ...
  /(slash commands?.*not guaranteed|prompt|\/job-hunter-ai.*unavailable)/i.test(agentsDoc)
) {
...
{
  const fixtureRoot = mkdtempSync(join(tmpdir(), 'job-hunter-ai-skills-'));
  try {
    const canonicalDir = join(fixtureRoot, '.agents', 'skills', 'job-hunter-ai');
    const claudeDir = join(fixtureRoot, '.claude', 'skills', 'job-hunter-ai');
    const opencodeDir = join(fixtureRoot, '.opencode', 'skills', 'job-hunter-ai');
    mkdirSync(canonicalDir, { recursive: true });
    mkdirSync(claudeDir, { recursive: true });
    mkdirSync(opencodeDir, { recursive: true });

    const fixtureSkill = '---\nname: job-hunter-ai\n---\n\n# canonical skill\n';
    const pointer = '../../../.agents/skills/job-hunter-ai/SKILL.md';
...
// "../../../.agents/skills/job-hunter-ai/SKILL.md". That is bug #1051, and it hit
...

{
  try {
    const tracked = execSync('git ls-files', { cwd: ROOT, encoding: 'utf-8' })
      .split('\n')
      .filter((p) => /^\.[^/]+\/skills\/job-hunter-ai\/SKILL\.md$/.test(p))
...
{
  const fixtureRoot = mkdtempSync(join(tmpdir(), 'job-hunter-ai-ensure-skills-'));
  try {
    const canonicalDir = join(fixtureRoot, '.agents', 'skills', 'job-hunter-ai');
    const claudeDir = join(fixtureRoot, '.claude', 'skills', 'job-hunter-ai');
    mkdirSync(canonicalDir, { recursive: true });
    ...
    const grokSkill = readFileSync(join(fixtureRoot, '.grok', 'skills', 'job-hunter-ai', 'SKILL.md'), 'utf-8');
    const claudeSkill = readFileSync(join(claudeDir, 'SKILL.md'), 'utf-8');
...

{
  ...
  const isolatedRoot = mkdtempSync(join(tmpdir(), 'job-hunter-ai-updater-standalone-'));
  try {
...
{
  const fixtureRoot = mkdtempSync(join(tmpdir(), 'job-hunter-ai-skills-unreadable-'));
  try {
    const canonicalDir = join(fixtureRoot, '.agents', 'skills', 'job-hunter-ai');
    const claudeDir = join(fixtureRoot, '.claude', 'skills', 'job-hunter-ai');
    mkdirSync(canonicalDir, { recursive: true });
...
{
  const fixtureRoot = mkdtempSync(join(tmpdir(), 'job-hunter-ai-skills-entry-dir-'));
  try {
    const canonicalDir = join(fixtureRoot, '.agents', 'skills', 'job-hunter-ai');
    const claudeDir = join(fixtureRoot, '.claude', 'skills', 'job-hunter-ai');
    const opencodeDir = join(fixtureRoot, '.opencode', 'skills', 'job-hunter-ai');
    ...
    const materialized = skills.materializeSkillEntrypoints(fixtureRoot);
    const opencodeSkill = readFileSync(join(opencodeDir, 'SKILL.md'), 'utf-8');
    if (JSON.stringify(materialized) === JSON.stringify(['.opencode/skills/job-hunter-ai/SKILL.md']) && opencodeSkill === fixtureSkill) {
...
{
  const fixtureRoot = mkdtempSync(join(tmpdir(), 'job-hunter-ai-skill-git-'));
  ...
  try {
    ...
    writeFileSync(join(claudeDir, 'SKILL.md'), pointer);
    writeFileSync(join(opencodeDir, 'SKILL.md'), pointer);
    gitRun(['add', '--', '.agents/skills/job-hunter-ai/SKILL.md']);

    const pointerBlob = gitRun(['hash-object', '-w', '--stdin'], { input: pointer });
    gitRun(['update-index', '--add', '--cacheinfo', `120000,${pointerBlob},.claude/skills/job-hunter-ai/SKILL.md`]);
    gitRun(['update-index', '--add', '--cacheinfo', `120000,${pointerBlob},.opencode/skills/job-hunter-ai/SKILL.md`]);
    ...
    const claudeIndex = gitRun(['ls-files', '-s', '--', '.claude/skills/job-hunter-ai/SKILL.md']);
    const opencodeIndex = gitRun(['ls-files', '-s', '--', '.opencode/skills/job-hunter-ai/SKILL.md']);
    ...
    const claudeBlob = gitRaw(['show', ':.claude/skills/job-hunter-ai/SKILL.md']);
    const opencodeBlob = gitRaw(['show', ':.opencode/skills/job-hunter-ai/SKILL.md']);
  ...
  // is where a silently-inferred age would actually do damage.
  {
    ...
    try {
      ...
      const e2eOut = execFileSync(NODE, [join(e2eTmp, 'followup-cadence.mjs')], {
        ...
        encoding: 'utf-8',
        timeout: 30000,
        env: { ...process.env, job_hunter_ai_PROFILE: '' },
      });
...
// ── 14b. ADD-ENTRY (/job-hunter-ai add) ────────────────────────────────
...
try {
  ...
  const cliTmp = mkdtempSync(join(tmpdir(), 'job-hunter-ai-add-cli-'));
  try {
    ...
    const env = { ...process.env, job_hunter_ai_CV: cvPath, job_hunter_ai_ARTICLE_DIGEST: adPath };
...
try {
  ...
  const tmpDir = mkdtempSync(join(tmpdir(), 'job-hunter-ai-migrate-'));
  try {
    ...
    run(NODE, ['merge-tracker.mjs', '--migrate'], { env: { ...process.env, job_hunter_ai_TRACKER: tracker } });
  ...
  const followupTmp = mkdtempSync(join(tmpdir(), 'job-hunter-ai-followup-link-'));
  try {
...
// a temp dir via the job_hunter_ai_REPORTS_DIR override.
...
try {
  ...
  const reserveRun = (args, dir, tracker = join(dir, 'applications.md')) => execFileSync(NODE, [RESERVE, ...args], {
    encoding: 'utf-8',
    env: { ...process.env, job_hunter_ai_REPORTS_DIR: dir, job_hunter_ai_TRACKER: tracker },
  }).trim();

  // Importing the module must expose the same allocator used by the CLI,
  // without running the CLI as an import side effect.
  const apiTmp = mkdtempSync(join(tmpdir(), 'job-hunter-ai-reserve-api-'));
  ...
  const apiProbe = execFileSync(NODE, ['--input-type=module', '--eval', `
    ...
    const nums = await api.reserveReportNumbers(1, {
      reportsDir: process.env.job_hunter_ai_REPORTS_DIR,
      trackerPath: process.env.job_hunter_ai_TRACKER,
    });
    const sentinel = join(process.env.job_hunter_ai_REPORTS_DIR, '001-RESERVED.md');
    let firstToken = null;
  ...
  `], {
    encoding: 'utf-8',
    env: { ...process.env, job_hunter_ai_REPORTS_DIR: apiTmp, job_hunter_ai_TRACKER: apiTracker },
  }).trim();
  let apiResult = null;
  ...
  const reserveTmp = mkdtempSync(join(tmpdir(), 'job-hunter-ai-reserve-'));
  const single = reserveRun([], reserveTmp);
  if (single === '001' && existsSync(join(reserveTmp, '001-RESERVED.md'))) {
    pass('job_hunter_ai_REPORTS_DIR override redirects sentinel to temp dir');
  } else {
  ...
  const trackerTmp = mkdtempSync(join(tmpdir(), 'job-hunter-ai-reserve-tracker-'));
  ...
  const fourDigitTmp = mkdtempSync(join(tmpdir(), 'job-hunter-ai-reserve-4digit-'));
  ...
  const unsafeRangeTmp = mkdtempSync(join(tmpdir(), 'job-hunter-ai-reserve-unsafe-range-'));
  ...
  const rangeTmp = mkdtempSync(join(tmpdir(), 'job-hunter-ai-reserve-range-'));
  ...
  const collideTmp = mkdtempSync(join(tmpdir(), 'job-hunter-ai-reserve-collide-'));
  ...
  const highRangeTmp = mkdtempSync(join(tmpdir(), 'job-hunter-ai-reserve-high-range-'));
  ...
  while (reserveRetries >= 0) {
    const concTmp = mkdtempSync(join(tmpdir(), 'job-hunter-ai-reserve-conc-'));
    try {
      const spawnReserve = () => new Promise(resolve => {
        const child = spawn(NODE, [RESERVE, '--count', '4'], {
          env: { ...process.env, job_hunter_ai_REPORTS_DIR: concTmp },
        });
        let stdout = '';
  ...
  const reserveRunFail = (args, dir) => {
    try {
      execFileSync(NODE, [RESERVE, ...args], {
        encoding: 'utf-8',
        stdio: ['pipe', 'pipe', 'pipe'],
        env: { ...process.env, job_hunter_ai_REPORTS_DIR: dir, job_hunter_ai_TRACKER: join(dir, 'applications.md') },
      });
      return null;
    } catch (err) {
      return err.status;
  ...
  const relTmp = mkdtempSync(join(tmpdir(), 'job-hunter-ai-reserve-release-'));
...
try {
  const vpTmp = mkdtempSync(join(tmpdir(), 'job-hunter-ai-verify-reports-'));
  try {
    const vpReports = join(vpTmp, 'reports');
    mkdirSync(vpReports, { recursive: true });
    const vpTracker = join(vpTmp, 'applications.md');
    const vpEnv = { ...process.env, job_hunter_ai_TRACKER: vpTracker, job_hunter_ai_REPORTS: vpReports };
...
try {
  const orTmp = mkdtempSync(join(tmpdir(), 'job-hunter-ai-verify-orphan-'));
  try {
    const orReports = join(orTmp, 'reports');
    mkdirSync(orReports, { recursive: true });
    const orTracker = join(orTmp, 'applications.md');
    const orEnv = { ...process.env, job_hunter_ai_TRACKER: orTracker, job_hunter_ai_REPORTS: orReports };
...
try {
  const dupNumTmp = mkdtempSync(join(tmpdir(), 'job-hunter-ai-verify-dupnum-'));
  try {
    const dupNumTracker = join(dupNumTmp, 'applications.md');
    const dupNumEnv = { ...process.env, job_hunter_ai_TRACKER: dupNumTracker };
  ...
  const cleanTmp = mkdtempSync(join(tmpdir(), 'job-hunter-ai-verify-dupnum-clean-'));
  try {
    ...
    const cleanOut = run(NODE, ['verify-pipeline.mjs'], { env: { ...process.env, job_hunter_ai_TRACKER: cleanTracker }, stdio: ['pipe', 'pipe', 'pipe'] });
...
try {
  ...
  const dedupTmp = mkdtempSync(join(tmpdir(), 'job-hunter-ai-dedup-'));
  try {
    ...
    const dedupResult = run(NODE, ['dedup-tracker.mjs'], { env: { ...process.env, job_hunter_ai_TRACKER: tracker } });
    if (dedupResult === null) {
...
try {
  const viaDedupTmp = mkdtempSync(join(tmpdir(), 'job-hunter-ai-dedup-via-'));
  try {
    ...
    const r = run(NODE, ['dedup-tracker.mjs'], { env: { ...process.env, job_hunter_ai_TRACKER: tracker } });
    if (r === null) {
...
try {
  const vpKeyTmp = mkdtempSync(join(tmpdir(), 'job-hunter-ai-verify-unicode-'));
  try {
    ...
    const vpKeyEnv = {
      ...process.env, job_hunter_ai_TRACKER: vpKeyTracker, job_hunter_ai_REPORTS: vpKeyReports,
    };
...
try {
  const rebuildTmp = mkdtempSync(join(tmpdir(), 'job-hunter-ai-rebuild-'));
  try {
...
try {
  const locTmp = mkdtempSync(join(tmpdir(), 'job-hunter-ai-dedup-loc-'));
  try {
...
try {
  const mergeTmp = mkdtempSync(join(tmpdir(), 'job-hunter-ai-merge-'));
  try {
    ...
    const mergeResult = run(NODE, ['merge-tracker.mjs'], { env: { ...process.env, job_hunter_ai_TRACKER: tracker, job_hunter_ai_ADDITIONS: additionsDir } });
    if (mergeResult === null) {
...
try {
  const clobberTmp = mkdtempSync(join(tmpdir(), 'job-hunter-ai-clobber-'));
  try {
    ...
    const clobberResult = run(NODE, ['merge-tracker.mjs'], { env: { ...process.env, job_hunter_ai_TRACKER: tracker, job_hunter_ai_ADDITIONS: additionsDir } });
...
try {
  ...
  const tier2Tmp = mkdtempSync(join(tmpdir(), 'job-hunter-ai-tier2-'));
  try {
    ...
    const tier2Result = run(NODE, ['merge-tracker.mjs'], { env: { ...process.env, job_hunter_ai_TRACKER: tracker, job_hunter_ai_ADDITIONS: additionsDir } });
    if (tier2Result === null) {
...
try {
  const viaTmp = mkdtempSync(join(tmpdir(), 'job-hunter-ai-via-'));
  try {
    ...
    const viaResult = run(NODE, ['merge-tracker.mjs'], { env: { ...process.env, job_hunter_ai_TRACKER: tracker, job_hunter_ai_ADDITIONS: additionsDir } });
    if (viaResult === null) {
...
try {
  ...
  const coTmp = mkdtempSync(join(tmpdir(), 'job-hunter-ai-nonlatin-co-'));
  try {
    ...
    const coResult = run(NODE, ['merge-tracker.mjs'], { env: { ...process.env, job_hunter_ai_TRACKER: tracker, job_hunter_ai_ADDITIONS: additionsDir } });
    if (coResult === null) {
...
try {
  ...
  const colTmp = mkdtempSync(join(tmpdir(), 'job-hunter-ai-colorder-'));
  try {
...
try {
  const runPdfSyncFixture = (name, trackerRow, pdfIndex = null, additions = []) => {
    const tmp = mkdtempSync(join(tmpdir(), `job-hunter-ai-merge-pdf-${name}-`));
    ...
    const result = run(NODE, ['merge-tracker.mjs'], {
      env: { ...process.env, job_hunter_ai_TRACKER: tracker, job_hunter_ai_ADDITIONS: additionsDir },
    });
    const merged = readFileSync(tracker, 'utf-8');
...
try {
  const col912Tmp = mkdtempSync(join(tmpdir(), 'job-hunter-ai-merge-912-'));
  try {
    ...
    const col912Result = run(NODE, ['merge-tracker.mjs'], {
      env: { ...process.env, job_hunter_ai_TRACKER: col912Tracker, job_hunter_ai_ADDITIONS: col912Additions },
    });
...
try {
  const staleNumTmp = mkdtempSync(join(tmpdir(), 'job-hunter-ai-merge-1704-'));
  try {
    ...
    const staleNumResult = run(NODE, ['merge-tracker.mjs'], {
      env: { ...process.env, job_hunter_ai_TRACKER: staleNumTracker, job_hunter_ai_ADDITIONS: staleNumAdditions },
    });
...
try {
  const reservedTmp = mkdtempSync(join(tmpdir(), 'job-hunter-ai-merge-reserved-'));
  try {
    ...
    const preserveResult = run(NODE, ['merge-tracker.mjs'], {
      env: { ...process.env, job_hunter_ai_TRACKER: reservedTracker, job_hunter_ai_ADDITIONS: reservedAdditions },
    });
...
try {
  const hyphenTmp = mkdtempSync(join(tmpdir(), 'job-hunter-ai-dedup-hyphen-'));
  try {
    ...
    const hOut = run(NODE, ['merge-tracker.mjs'], {
      env: { ...process.env, job_hunter_ai_TRACKER: hTracker, job_hunter_ai_ADDITIONS: hAdditions },
    });
    ...
    try {
      badOut = execFileSync(NODE, ['verify-pipeline.mjs'], {
        ...
        env: { ...process.env, job_hunter_ai_TRACKER: hBadRow, job_hunter_ai_REPORTS: hReports },
      });
    } catch (e) {
    ...
    try {
      hdrOut = execFileSync(NODE, ['verify-pipeline.mjs'], {
        cwd: ROOT, encoding: 'utf-8', timeout: 30000,
        env: { ...process.env, job_hunter_ai_TRACKER: hHeaderish, job_hunter_ai_REPORTS: hReports },
      });
    } catch (e) {
...
try {
  const reqTmp = mkdtempSync(join(tmpdir(), 'job-hunter-ai-merge-1524-'));
  try {
    ...
    const reqResult = run(NODE, ['merge-tracker.mjs'], { env: { ...process.env, job_hunter_ai_TRACKER: reqTracker, job_hunter_ai_ADDITIONS: reqAdditions } });
    if (reqResult === null) {
...
try {
  let retries = 1;
  while (retries >= 0) {
    const mergeTmp = mkdtempSync(join(tmpdir(), 'job-hunter-ai-merge-lock-'));
    /**
    ...
    function spawnMerge(additionsDir, holdMs = 0) {
      ...
      const result = new Promise(resolve => {
        const child = spawn(NODE, ['merge-tracker.mjs'], {
          ...
          env: {
            ...process.env,
            job_hunter_ai_TRACKER: join(mergeTmp, 'data', 'applications.md'),
            job_hunter_ai_ADDITIONS: additionsDir,
            job_hunter_ai_TRACKER_LOCK: join(mergeTmp, 'job-hunter-ai-merge-tracker-fixture.lock'),
            job_hunter_ai_MERGE_HOLD_MS: String(holdMs),
            job_hunter_ai_MERGE_READY_IPC: '1',
          },
...
if (!sqliteAvailable) {
  ...
  try {
    const idxTmp = mkdtempSync(join(tmpdir(), 'job-hunter-ai-index-'));
    try {
      const md = join(idxTmp, 'applications.md');
      const env = { ...process.env, job_hunter_ai_TRACKER: md };
...
try {
  ...
  // Release Please tags carry the component prefix (job-hunter-ai-v1.9.0); the
  // prefix must be stripped or the releases-API fallback is dead code (#923).
  if (parse('job-hunter-ai-v1.9.0') === '1.9.0') {
    pass('SEMVER_RE parses Release Please component-prefixed tag (job-hunter-ai-v1.9.0 → 1.9.0)');
  } else {
    fail(`SEMVER_RE failed on job-hunter-ai-v1.9.0 (got ${parse('job-hunter-ai-v1.9.0')}) — releases-API fallback is dead code (#923)`);
  }
  ...
  if (parse('job-hunter-ai') === null && parse('v1.9') === null) {
    pass('SEMVER_RE rejects non-semver input');
  } else {
    fail(`SEMVER_RE matched non-semver input (job-hunter-ai → ${parse('job-hunter-ai')}, v1.9 → ${parse('v1.9')})`);
  }
} catch (e) {
...
try {
  ...
  if (compileOnlyValidation.issues.length === 0) {
    pass('--compile-only validation accepts minimal user .tex without job-hunter-ai macros');
  } else {
  ...
  if (strictValidation.issues.some(i => /section|resumeSubheading|pdfgentounicode/i.test(i))) {
    pass('default validation still enforces job-hunter-ai template checks');
  } else {
...
try {
  ...
  const goodEntry = { name: 'job-hunter-ai-plugin-x', id: 'x', repo: 'https://github.com/a/job-hunter-ai-plugin-x', author: 'a', hooks: ['ingest'], requiredEnv: [], allowedHosts: ['api.x.com'], license: 'MIT', version: '1.0.0', sha: 'a'.repeat(40) };
  ...
  if (reg.validateRegistryEntry({ ...goodEntry, name: 'evil-x' }, regOpts).length > 0) pass('registry: name must start with job-hunter-ai-plugin-');
  ...
  writeFileSync(join(succTmp, 'plugins-registry.json'), JSON.stringify({ registryVersion: 1, plugins: [{ name: 'job-hunter-ai-plugin-gmail', id: 'gmail', repo: 'https://github.com/a/job-hunter-ai-plugin-gmail', author: 'a', hooks: ['ingest'], requiredEnv: [], allowedHosts: [], license: 'MIT', version: '2.0.0', sha: SUCC_SHA, supersedesBundled: true }] }));
  ...
  if (reg.successorFor(succTmp, 'gmail')?.name === 'job-hunter-ai-plugin-gmail') pass('successor: successorFor() surfaces the maintained version of a bundled id');
  else fail('successor: successorFor should return the registered successor');
  rmSync(succTmp, { recursive: true, force: true });

  if (install.parseRepoArg('alice/job-hunter-ai-plugin-foo').id === 'foo') pass('install: owner/job-hunter-ai-plugin-foo parses to id "foo"');
  ...
  if (nameRej) pass('install: refuses a repo not named job-hunter-ai-plugin-*');
...
try {
  ...
  const webStatusLists = [
    ...
    // 55.3b+ the degraded-path FALLBACK in the states ACL (job-hunter-ai-ui's find, #2282):
  ...
  // doctorState() in web/src/lib/job-hunter-ai.ts hand-copies USER_LAYER_PREREQS
  ...
  // #2282, different symptom (job-hunter-ai-ui's census, 31-jul).
  {
    ...
    const webDoctorPath = join(ROOT, 'web', 'src', 'lib', 'job-hunter-ai.ts');
  ...
  const webParserPath = join(ROOT, 'web', 'src', 'lib', 'job-hunter-ai.ts');
  if (existsSync(webParserPath)) {
    ...
    if (missingCols.length === 0) {
      pass('web/src/lib/job-hunter-ai.ts still references the essential tracker columns');
    } else {
      fail(`web parser no longer references column(s): ${missingCols.join(', ')} — core and web drifted`);
    }
  } else {
    warn('web/src/lib/job-hunter-ai.ts not found — web layer moved? update contract freeze section');
  }
} catch (e) {
...
try {
  ...
  if (
    titlesMode.includes('/job-hunter-ai scan') &&
...
for (const skillPath of ['.claude/skills/job-hunter-ai/SKILL.md', '.agents/skills/job-hunter-ai/SKILL.md']) {
  ...
  if (
    ...
    skill.includes('/job-hunter-ai titles') &&
    /Standalone modes[\s\S]*Applies to:[^\n]*`titles`/.test(skill)
  ) {
    pass(`${skillPath} exposes /job-hunter-ai titles in argument-hint, routing, discovery, and standalone loading`);
  } else {
    fail(`${skillPath} does not fully expose /job-hunter-ai titles`);
  }
}
...
console.log('\n59. CV template resolver (cv-templates.mjs)');
{
  ...
  const noProfile = { env: { ...process.env, job_hunter_ai_PROFILE: join(tmpdir(), 'job-hunter-ai-no-such-profile.yml') } };
