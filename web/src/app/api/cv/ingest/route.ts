...
function ingestPrompt(source: string): string {
  ...
  return `You convert a person's CV into clean cv.md markdown that EXACTLY mirrors job-hunter-ai's reference format.
...
export async function POST(req: Request) {
  ...
  try {
    if (ctype.includes("application/json")) {
      ...
      const dir = fs.mkdtempSync(path.join(os.tmpdir(), "job-hunter-ai-cv-"));
