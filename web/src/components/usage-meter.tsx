...
// Soft budgets (tunable via localStorage `job-hunter-ai:usage-budget`). The bar
...
export function UsageMeter() {
  ...
  useEffect(() => {
    try {
      const cfg = localStorage.getItem("job-hunter-ai:config");
      setCli(cfg ? JSON.parse(cfg).cliId || null : null);
      const b = localStorage.getItem("job-hunter-ai:usage-budget");
